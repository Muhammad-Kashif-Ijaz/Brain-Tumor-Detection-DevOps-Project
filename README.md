...
The high-fidelity volumetric path uses MONAI Model Zoo's `brats_mri_segmentation` bundle version `0.5.4`. It is designed for four aligned MRI volumes: T1c, T1, T2, and FLAIR, and returns whole-tumor, tumor-core, and enhancing-tumor segmentation. This is the appropriate route when coverage across the whole scan and all anatomical planes matters. The service canonicalizes the aligned study and, when needed, resamples it to the model's 1 mm grid before review. Each study is then reviewed across all eight mirrored spatial orientations, averaged back into one 3D prediction, and cleaned only for isolated voxel noise. Separate candidate regions are retained rather than reduced to one largest region. The bundled review image contains sagittal, coronal, axial, and all-detected-regions overview panels.
...
Image, video, and camera modes use the MIT-licensed [brain MRI SegFormer-B2 checkpoint](https://huggingface.co/kiselyovd/brain-mri-segmentation). It was trained for binary tumor segmentation on lower-grade glioma MRI slices with a patient-level test split. The service standardizes the detected brain field before inference and averages original and horizontally mirrored predictions before mask cleanup. These modes remain slice-level research review only; a photograph, video, or single screenshot cannot provide the clinical coverage or reliability of a complete aligned 3D MRI examination.
